...
# MAP controller lives in its own module so the original pure-PP Controller.py
# stays byte-untouched (loaded by the original controller_manager exec).
from controller.combined.src.map_controller import Controller, parse_l1_sectors
from controller.ftg.ftg import FTG
from controller.offroad_planner.offroad_planner import OffRoadController, PlannerConfig

# MAP steering lookup lives in its own (optionally built) package; keep the
# import soft so the controller still runs as pure PP when it is absent.
try:
    from steering_lookup.lookup_steer_angle import LookupSteerAngle
except ImportError:
    LookupSteerAngle = None
...
L1_PARAMS = [
    ...
    'ot_l1_hold_sec', 'ot_l1_release_sec',   # post-avoidance L1 floor hold/ramp
]

# fallbacks for L1_PARAMS a map yaml may predate. 0.0/0.0 = no hold, no ramp:
# the L1 floor snaps back the tick OVERTAKE ends (pre-hold behaviour).
L1_DEFAULTS = {
    'ot_l1_hold_sec': 0.0,
    'ot_l1_release_sec': 0.0,
}

# per-map OVERTAKE L1 floor sectors. Not in L1_PARAMS because it is a double
# ARRAY, not a scalar: flat [s_start, s_end, t_clip_min, ...] triples, see
# map_controller.parse_l1_sectors. Empty (all-zero) triple -> feature off.
OT_SECTORS_PARAM = 'ot_sectors'
...
OFFROAD_PARAM_MAP = {
    ...
    'offroad_use_early_curvature_quintic': 'use_early_curvature_quintic',
    'offroad_early_curvature_gain': 'early_curvature_gain',
...
# /controller/debug layout (measure:=true only). Matches controller_manager.py's
# DEBUG_FIELDS - map_controller.py's self.dbg already fills these every tick,
# this node just wasn't publishing them yet.
DEBUG_FIELDS = [
    ...
    'l1_floor',           # m, L1 lower bound in use (shows which ot_sector fired)
]


class ControllerManager(Node):
    """ROS2 port of the ROS1 unicorn controller_manager. Lateral control is
    Pure-Pursuit by default; the ROS1 MAP / steering-lookup branch is available
    behind the live-tunable `use_map` param (table chosen via `LU_table`).
    ...
    def __init__(self):
        ...
        # MAP steering lookup (on/off + table selection; both live-tunable).
        # Load failures never kill the node: use_map just falls back to False.
        self.use_map = bool(self._get_param('use_map', False))
        self.LU_table = self._get_param('LU_table', 'UNICORN2-0410')
        self.steer_lookup = self._load_steer_lookup(self.LU_table) if self.use_map else None
        if self.use_map and self.steer_lookup is None:
            self.use_map = False
        ...
        # L1_DEFAULTS covers the params added after the older map yamls were
        # written, so those still load instead of erroring out to None.
        for p in L1_PARAMS:
            setattr(self, p, self._get_param(p, L1_DEFAULTS.get(p)))
        # OVERTAKE L1 floor sectors: off unless the map's yaml lists sectors.
        # The default is a zeroed triple, not [], so the param declares as a
        # DOUBLE_ARRAY (rclpy cannot infer the type of an empty list).
        self.ot_sectors = list(self._get_param(OT_SECTORS_PARAM, [0.0, 0.0, 0.0]))
        ...
        if self.measuring:
            ...
            # did the chord cap bind? Layout is DEBUG_FIELDS above; publish it
    ...
    def _load_steer_lookup(self, model):
        """Build the MAP LookupSteerAngle for `model`; None (+ error log) on any failure."""
        if LookupSteerAngle is None:
            self.get_logger().error(
                f"[{self.name}] use_map requested but steering_lookup package is not "
                "installed (build it with colcon); falling back to PP")
            return None
        try:
            lookup = LookupSteerAngle(str(model), self.get_logger().info)
            self.get_logger().info(f"[{self.name}] MAP steering lookup loaded: {model}")
            return lookup
        except Exception as e:
            self.get_logger().error(f"[{self.name}] MAP lookup load failed ({e}); falling back to PP")
            return None
    ...
    def global_wpnts_cb(self, data: WpntArray):
        ...
        self.controller = Controller(
            ...
            ot_sectors=self.ot_sectors,
            ot_l1_hold_sec=self.ot_l1_hold_sec,
            ot_l1_release_sec=self.ot_l1_release_sec,
    ...
    def dyn_param_cb(self, params):
        ...
        for param in params:
            ...
            if name in L1_PARAMS:
            ...
            elif name == OT_SECTORS_PARAM:
                # array param: re-parse into triples before handing it over
                self.ot_sectors = list(param.value)
                if self.controller is not None:
                    self.controller.ot_sectors = parse_l1_sectors(self.ot_sectors)
                ...
                # live MAP<->PP toggle; lazily (re)load the lookup on enable.
                # Never rejects the set: on load failure it just stays on PP.
                self.use_map = bool(param.value)
                if self.use_map and self.steer_lookup is None:
                    self.steer_lookup = self._load_steer_lookup(self.LU_table)
                    if self.steer_lookup is None:
                        self.use_map = False
                if self.controller is not None:
                    self.controller.steer_lookup = self.steer_lookup
                    self.controller.use_map = self.use_map
                self.get_logger().info(
                    f"[{self.name}] lateral control: {'MAP' if self.use_map else 'PP'}")
            elif name == 'LU_table':
                self.LU_table = param.value
                if self.use_map:  # hot-swap table; keep old lookup if load fails
                    new_lookup = self._load_steer_lookup(self.LU_table)
                    if new_lookup is not None:
                        self.steer_lookup = new_lookup
                        if self.controller is not None:
                            self.controller.steer_lookup = new_lookup
    ...
    def save_yaml(self):
        ...
        try:
            ...
            params[OT_SECTORS_PARAM] = [float(v) for v in self.ot_sectors]
            params['use_map'] = bool(self.use_map)    # MAP steering lookup state
            params['LU_table'] = str(self.LU_table)
    ...
    def behavior_cb(self, data: BehaviorStrategy):
        ...
        if (
            previous_state == "OFFROADONLY"
            and self.state != "OFFROADONLY"
            and self.controller is not None
        ):
            cfg = self.offroad_controller.config
            self.controller.latch_offroad_l1_floor(
                cfg.lookahead_min_m,
                cfg.lookahead_speed_gain_s,
                self.speed_now,
            )
    ...
    def _publish_offroad_debug(self, result):
        ...
        for candidate in result.candidates:
            ...
            # Only up to the first collision: the tail beyond it runs through
            # the obstacle or off the road and reads as a planned path outside
            # the map. A collision-free candidate is drawn whole.
            marker.points = [Point(x=float(x), y=float(y), z=0.0)
                             for x, y in zip(candidate.x[:candidate.valid_count],
                                             candidate.y[:candidate.valid_count])]
        ...
        marker.points = [Point(x=float(x), y=float(y), z=0.02)
                         for x, y in zip(chosen.x[:chosen.valid_count],
                                         chosen.y[:chosen.valid_count])]
