...
FTG. The optional quintic fan is an extension of the paper algorithm: every
candidate starts at the measured vehicle pose, fans out with a different
bounded initial steering angle, and reaches a common terminal pose parallel to
the base frame.
"""
...
class PlannerConfig:
    ...
    speed_horizon_gain_s: float = 0.40       # k_v in equation (13)
    min_maneuver_m: float = 1.40             # delta-s_min in equation (13)
    ...
    def validated(self) -> "PlannerConfig":
        ...
        if not np.isfinite(self.fan_goal_offset_m):
            raise ValueError("off-road fan goal offset must be finite")
        if (isinstance(self.fan_steering_samples, bool)
                or int(self.fan_steering_samples) != self.fan_steering_samples
                or self.fan_steering_samples < 3
                or self.fan_steering_samples % 2 == 0):
            raise ValueError("off-road fan steering samples must be an odd integer >= 3")
        return self
...
class CandidatePath:
    ...
    generation_mode: str = "cubic_offset"
...
class OffRoadPlanner:
    ...
    def _quintic_coefficients(
        ...
        """Return q(s) coefficients satisfying position/slope/curvature ends."""
        h = float(distance)
        a0 = float(q0)
        a1 = float(dq0)
        a2 = 0.5 * float(ddq0)
        matrix = np.asarray([
            [h**3, h**4, h**5],
            [3.0 * h**2, 4.0 * h**3, 5.0 * h**4],
            [6.0 * h, 12.0 * h**2, 20.0 * h**3],
        ])
        rhs = np.asarray([
            float(qf) - (a0 + a1 * h + a2 * h**2),
            float(dqf) - (a1 + 2.0 * a2 * h),
            float(ddqf) - 2.0 * a2,
        ])
    ...
    @staticmethod
    def _lateral_second_derivative_for_curvature(
        q: float,
        dq: float,
        base_curvature: float,
        path_curvature: float,
    ) -> float:
        """Invert equation (6) to obtain q'' for a requested path curvature."""
        one_minus_qk = 1.0 - q * base_curvature
        if abs(one_minus_qk) <= _EPS:
            return math.copysign(math.inf, one_minus_qk)
        scale = math.hypot(dq, one_minus_qk)
        orientation_sign = 1.0 if one_minus_qk >= 0.0 else -1.0
        numerator = (
            scale**2 * (path_curvature * scale / orientation_sign - base_curvature)
            - base_curvature * dq**2
        )
        return numerator / one_minus_qk

    def _generate_fan_candidate(
        self,
        index: int,
        initial_steer_rad: float,
        current_s_mod: float,
        current_s_unwrapped: float,
        current_d: float,
        heading_error: float,
    ) -> CandidatePath:
        """Generate a quintic branch that rejoins the configured base-frame pose."""
        assert self.base_frame is not None
        cfg = self.config
        ds = np.arange(
            0.0, cfg.path_horizon_m + 0.5 * cfg.path_resolution_m,
        ...
        goal_distance = min(cfg.fan_goal_distance_m, cfg.path_horizon_m)
        goal_distance = max(goal_distance, cfg.path_resolution_m)
        dq0 = math.tan(float(np.clip(
            heading_error, -math.radians(75.0), math.radians(75.0))))
        base_curvature = float(self.base_frame.geometry(current_s_mod)[4])
        requested_curvature = math.tan(initial_steer_rad) / cfg.wheelbase_m
        ddq0 = self._lateral_second_derivative_for_curvature(
            current_d, dq0, base_curvature, requested_curvature)
        coefficients = self._quintic_coefficients(
            current_d, dq0, ddq0,
            cfg.fan_goal_offset_m, 0.0, 0.0,
            goal_distance,
        )

        u = np.minimum(ds, goal_distance)
        q = sum(coefficients[power] * u**power for power in range(6))
        dq = sum(
            power * coefficients[power] * u ** (power - 1)
            for power in range(1, 6)
        ...
        finished = ds >= goal_distance
        q[finished] = cfg.fan_goal_offset_m
        dq[finished] = 0.0
        ddq[finished] = 0.0

        return self._candidate_from_profile(
            index=index,
            final_offset=cfg.fan_goal_offset_m,
            current_s_mod=current_s_mod,
            current_s_unwrapped=current_s_unwrapped,
            ds=ds,
            q=q,
            dq=dq,
            ddq=ddq,
            generation_mode="quintic_fan",
            initial_steer_rad=initial_steer_rad,
    ...
    def _candidate_from_profile(
        ...
        *,
        ...
        """Convert one Frenet lateral profile into a fully evaluated geometry."""
        assert self.base_frame is not None
        cfg = self.config
        ...
        candidate = CandidatePath(
            ...
            initial_steer_rad=float(initial_steer_rad),
    ...
    def _check_collision(self, path: CandidatePath, obstacle_points: np.ndarray) -> None:
        ...
        if obstacle_points.size:
            front_extent = cfg.vehicle_front_m + cfg.collision_margin_m
            rear_extent = cfg.vehicle_rear_m + cfg.collision_margin_m
            ...
            for j, (x, y, yaw) in enumerate(zip(path.x, path.y, path.yaw)):
                ...
                if np.any((longitudinal <= front_extent)
                          & (longitudinal >= -rear_extent)
    ...
    def _safety_costs(
        ...
        candidates: Optional[Sequence[CandidatePath]] = None,
    ) -> np.ndarray:
        cfg = self.config
        if candidates and any(path.generation_mode == "quintic_fan" for path in candidates):
            # Fan candidates share a terminal offset, so lateral-offset index
            # distance no longer represents path proximity. Blur collision risk
            # using the maximum Cartesian separation between complete paths.
            count = len(candidates)
            separation = np.zeros((count, count), dtype=float)
            for i in range(count):
                for j in range(i + 1, count):
                    ...
                    separation[i, j] = separation[j, i] = float(np.max(distance))
            weights = np.exp(-0.5 * (separation / cfg.safety_sigma_m) ** 2)
            return (weights @ collisions.astype(float)) / np.maximum(
                np.sum(weights, axis=1), _EPS)
    ...
    def plan(
        ...
        if cfg.use_quintic_fan:
            curvature_steer_limit = math.atan(cfg.wheelbase_m * cfg.max_curvature)
            steering_limit = min(cfg.max_steer_rad, curvature_steer_limit)
            steering_samples = np.linspace(
                -steering_limit,
                steering_limit,
                int(cfg.fan_steering_samples),
            )
            fan_candidates = [
                self._generate_fan_candidate(
                    len(candidates) + i, float(steer), current_s, current_s_unwrapped,
                    current_d, heading_error,
                )
                for i, steer in enumerate(steering_samples)
            ]
            candidates.extend(fan_candidates)
        ...
        safety = self._safety_costs(collision_flags, candidates)
