...
class ObstacleSpliner(Node):
    ...
    def do_spline(self, obs: Obstacle, gb_wpnts: WpntArray) -> Tuple[WpntArray, MarkerArray]:
        ...
        if obs.is_static == True:
            ...
            # the seed/apex points wildly apart, blowing up `l` and therefore nSamples --
            # the np.linspace/np.zeros below then try to allocate tens of GB and kill the
            # node. Bail out the same way the pre_dist check above does instead of crashing.
