...
class StateMachine(Node):
    ...
    def __init__(self) -> None:
        ...
        self.cur_recovery_wpnts.ggv_scale_factor = 1.0
        self.cur_static_avoidance_wpnts.ggv_scale_factor = 1.0
        # Global Frenet-s interval where static OVERTAKE uses a stricter local
        # tire-acceleration limit. Both endpoints are inclusive.
        self.cur_static_avoidance_wpnts.ggv_scale_s_range = (34.0, 40.0, 1.0)
        ...
        # threshold. Without this the machine enters OVERTAKE on every burst and
    ...
    def save_start_traj_cb(self, msg):
        if len(self.cur_start_wpnts_candidate.wpnts) != 0:
            self.update_velocity(self.cur_start_wpnts_candidate, self.cur_start_wpnts.ggv_scale_factor)
    ...
    def recovery_wpnts_cb(self, data: WpntArray):
        if len(data.wpnts) != 0:
            self.update_velocity(data, self.cur_recovery_wpnts.ggv_scale_factor)
    ...
    def avoidance_cb(self, data: OTWpntArray):
        if len(data.wpnts) != 0:
            self.update_velocity(data, self.cur_avoidance_wpnts.ggv_scale_factor)
    ...
    def static_avoidance_cb(self, data: OTWpntArray):
        if len(data.wpnts) != 0:
            self.update_velocity(
                data,
                self.cur_static_avoidance_wpnts.ggv_scale_factor,
                self.cur_static_avoidance_wpnts.ggv_scale_s_range,
            )
    ...
    def _check_static_direct_controller_trigger(self, disabled: bool, label: str) -> bool:
        ...
        # ``loop()`` intentionally clears every path cache's closest_target
        # immediately before running transitions. The target that was actually
        # published for TRAILING in the previous 80-Hz loop therefore is the
        # authoritative selection here. Re-match its ID against this loop's
        # obstacle snapshot so a disappeared/stale target cannot trigger OFFROAD.
        trailing_targets = getattr(
            self.behavior_strategy, "trailing_targets", []
        )
        if not trailing_targets:
            return False

        previous_target = trailing_targets[0]
        previous_id = getattr(previous_target, "id", None)
        target = next(
            (
                obs for obs in self.cur_obstacles_in_interest
                if getattr(obs, "id", None) == previous_id
            ),
            None,
        )
        if target is None or not bool(getattr(target, "is_static", False)):
            return False

        gap = (
            float(target.s_start) - float(self.cur_s)
        ) % float(self.track_length)
        within_trigger_distance = gap <= self.offroad_distance_m
        if within_trigger_distance:
            self.get_logger().warn(
                f"[{self.name}] {label} trigger (distance): "
                f"static target gap={gap:.2f} m, speed={self.cur_vs:.2f} m/s",
                throttle_duration_sec=0.5,
            )
        return within_trigger_distance
    ...
    # HELPER FUNCS #
    ################
    def update_velocity(self, wpnts_msg, ggv_scale_factor=1.0, ggv_scale_s_range=None):
        ...
        # GGV columns are [v, ax_max, ay_max]. A uniform factor can be applied
        # directly to both acceleration columns. For a Frenet-s range override,
        # use the planner's waypoint-local ``mu`` input: it multiplies the same
        # GGV ax/ay limits at every waypoint and lets the forward/backward solver
        # propagate the required acceleration/deceleration across range edges.
        # The independent drivetrain acceleration/braking tables stay unchanged.
        ggv_scaled = self.ggv.copy()
        local_ggv_scale = None
        if ggv_scale_s_range is None:
            ggv_scaled[:, 1:3] *= ggv_scale_factor
        else:
            s_start, s_end, range_scale = ggv_scale_s_range
            waypoint_s = np.array([wp.s_m for wp in wpnts], dtype=float)
            local_ggv_scale = np.full(len(wpnts), ggv_scale_factor, dtype=float)
            if s_start <= s_end:
                in_range = (waypoint_s >= s_start) & (waypoint_s <= s_end)
            else:
                # Also support a range crossing the track's s=0 boundary.
                in_range = (waypoint_s >= s_start) | (waypoint_s <= s_end)
            local_ggv_scale[in_range] = range_scale
